...
void SiPixelDigiMorphing::morph(uint64_t* imap, uint64_t* omap,
										  uint64_t* kernel, MorphOption op) {
  ...
  for(int roc=0; roc<nrocs_; roc++, o+=2*iters_) {
	 for(int row=0; row<nrows_; row++, o++) {
		...
		if ((valid&validMask)!=0) {
		  ...
		  for (int jj=0; jj<ksize_; jj++) {
			 for (int ii=0; ii<ksize_; ii++) {
				...
				if (op==Erode) v^=(kernel[ii]<<jj);
				uint64_t vv=v;
				for(int b=1; b<ksize_; b++) vv|=(v>>b);
				*o|=((vv<<iters_)&m[jj]);
			 }
			 if (op==Erode) *o^=m[jj];
		  }
		...
		for(int ii=0; ii<ksize_; ii++) i[ii]++;
