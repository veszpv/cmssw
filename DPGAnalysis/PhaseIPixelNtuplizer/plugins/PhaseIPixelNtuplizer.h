...
class PhaseIPixelNtuplizer : public edm::EDAnalyzer {
 ...
 private:
  ...
  void getEvtData(const edm::Event& iEvent, 
  ...
  std::map<reco::TrackRef, TrackData> 
    getTrackData(const edm::Handle<reco::VertexCollection>& vertexCollectionHandle, 
		 const edm::Handle<TrajTrackAssociationCollection>& trajTrackCollectionHandle);

  void getTrajTrackData(const edm::Handle<reco::VertexCollection>& vertexCollectionHandle, 
  ...
  void getTrajTrackData_Cosmics(const edm::Handle<reco::VertexCollection>& vertexCollectionHandle, 
				const edm::Handle<TrajTrackAssociationCollection>& 
				trajTrackCollectionHandle,
				const edm::Handle<edmNew::DetSetVector<SiPixelCluster>>&
				clusterCollectionHandle);

  void checkAndSaveTrajMeasurementData(const TrajectoryMeasurement& measurement, 
				       const edm::Handle<TrajTrackAssociationCollection>& 
				       trajTrackCollectionHandle, 
				       const edm::Handle<edmNew::DetSetVector<SiPixelCluster>>&
				       clusterCollectionHandle,
  ...
  std::vector<TrajectoryMeasurement>
    getLayer1ExtrapolatedHitsFromMeas(const TrajectoryMeasurement& trajMeasurement);

  void getDisk1PropagationData(const edm::Handle<TrajTrackAssociationCollection>& 
			       trajTrackCollectionHandle);

  void handleDefaultError(const std::string& exceptionType, const std::string& streamType, 
			  std::string msg);
 ...
 private:
  void propagateTrackToLayer1(const edm::Ref<std::vector<Trajectory>>& trajectory, 
			      const reco::TrackRef track);

  std::tuple<std::vector<TrajectoryMeasurement>::const_iterator, float> 
    findMatchingTrajMeasurement(const GlobalPoint& referencePoint, 
				const ModuleData& referenceModInfo, 
				const std::vector<TrajectoryMeasurement>& possibleMatches);

  const SiPixelCluster* 
    getClosestClusterOnDetSetToPoint(const edmNew::DetSet<SiPixelCluster>& clustersOnDet, 
				     const LocalPoint& referencePoint);

  float trajMeasGlobalPointDistanceSquared(const TrajectoryMeasurement& trajMeasurement, 
					   const GlobalPoint& referencePoint);

  float clusterPointDistanceSquared(const DetId& detId, const SiPixelCluster& cluster, 
				    const LocalPoint& referencePoint);

  LocalPoint clusterPointDistanceVector (const DetId& detId, const SiPixelCluster& cluster, 
					 const LocalPoint& referencePoint);

  float clusterPointDistance(const DetId& detId, const SiPixelCluster& cluster, 
			     const LocalPoint& referencePoint);

  void printTrackCompositionInfo(const edm::Ref<std::vector<Trajectory>>& trajectory, 
				 const reco::TrackRef& track, 
				 const edm::Handle<edmNew::DetSetVector<SiPixelCluster>> 
				 clusterCollectionHandle, 
				 const edm::Handle<reco::VertexCollection>& vertexCollectionHandle);
...
namespace NtuplizerHelpers  {
  ...
  reco::VertexCollection::const_iterator
    findClosestVertexToTrack(const reco::TrackRef& track, 
			     const edm::Handle<reco::VertexCollection>& vertexCollectionHandle, 
			     const unsigned int& minTracks = 0);
  ...
  void
  getClosestOtherTrajMeasurementDistanceByLooping(const TrajectoryMeasurement& measurement,
						  const edm::Handle<TrajTrackAssociationCollection>&
						  trajTrackCollectionHandle, float& distance, 
						  float& dx, float& dy);
